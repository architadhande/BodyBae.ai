...
class SmolLMHealthRAG:
    ...
    def __init__(self):
        ...
        # Initialize SmolLM3-3B model with optimizations
        try:
            ...
            logger.info("✅ SmolLM3-3B model loaded successfully on CPU")
            
        except Exception as e:
            logger.error(f"❌ Failed to load SmolLM3 model: {e}")
            # Fallback to smaller model or disable AI
...
BodyBae Response:"""
            ...
            # Generate response using SmolLM3 with error handling
            try:
                response = self.generator(
                    full_prompt,
                    max_new_tokens=250,  # Reduced for stability
                    temperature=0.7,
                    do_sample=True,
                ...
                response_markers = ["BodyBae Response:", "BodyBae:", "Response:"]
                ai_response = generated_text
                
                for marker in response_markers:
                    if marker in generated_text:
                        response_start = generated_text.find(marker) + len(marker)
                ...
                # Use retrieved context for fallback
                return self.create_context_based_response(user_message, relevant_context, user_profile)
    ...
    def create_context_based_response(self, user_message: str, context: str, user_profile: Dict = None) -> str:
        """Create intelligent response using retrieved context when AI generation fails"""
        name = user_profile.get('name', 'there') if user_profile else 'there'
        
        # Extract relevant information from context
        context_sentences = context.split('.')[:5]  # First 5 sentences
        relevant_info = '. '.join([s.strip() for s in context_sentences if len(s.strip()) > 20])
        
        # Create response based on context and user message
        message_lower = user_message.lower()
        
        response = f"Hi {name}! Based on the latest health science, here's what I can tell you:\n\n"
        
        if relevant_info:
            response += relevant_info[:400] + "...\n\n"  # Limit length
        
        # Add personalized recommendations
        if user_profile:
            goals = user_profile.get('goals', [])
            if goals:
                response += f"Given your goals ({', '.join(goals[:2])}), I recommend focusing on evidence-based approaches that align with your profile.\n\n"
        
        response += "Would you like me to elaborate on any specific aspect of this advice?"
        
        return response

    def get_fallback_response(self, user_message: str, user_profile: Dict = None) -> str:
        """Enhanced fallback response when all AI methods fail"""
        name = user_profile.get('name', 'there') if user_profile else 'there'
        
        # Try to retrieve some context even if AI fails
        try:
            context = self.retrieve_relevant_context(user_message, n_results=2)
            if context:
                return self.create_context_based_response(user_message, context, user_profile)
        except:
            pass
        
        # Final fallback
        return f"Hi {name}! I'm here to help with your health and fitness journey. While I'm experiencing some technical difficulties with my AI systems, I can still provide evidence-based advice. Could you please rephrase your question or ask about specific topics like nutrition, exercise, or wellness goals?"
